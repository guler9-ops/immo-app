...
router.get('/me', requireAuth, async (req, res) => {
  const user = await db.prepare('SELECT id, username, email, role, license_expires_at, created_at FROM users WHERE id = ?').get(req.user.id);
  res.json(user);
});
...
router.post('/activate', requireAuth, async (req, res) => {
  ...
  const lc = await db.prepare('SELECT * FROM license_codes WHERE code = ? AND used = 0').get(code.trim().toUpperCase());
