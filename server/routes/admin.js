...
router.post('/users', requireAuth, requireAdmin, async (req, res) => {
  ...
  const user = await db.prepare('SELECT id, username, email, role, license_expires_at, created_at FROM users WHERE id = ?').get(result.lastInsertRowid);
...
router.put('/users/:id', requireAuth, requireAdmin, async (req, res) => {
  ...
  if (license_expires_at) {
    await db.prepare('UPDATE users SET license_expires_at = ? WHERE id = ?').run(license_expires_at, user.id);
  }
  if (email !== undefined) {
    await db.prepare('UPDATE users SET email = ? WHERE id = ?').run(email, user.id);
  }
  res.json(await db.prepare('SELECT id, username, email, role, license_expires_at FROM users WHERE id = ?').get(user.id));
});
...
router.post('/license-codes', requireAuth, requireAdmin, async (req, res) => {
  ...
  do {
    code = generateCode();
    attempts++;
  } while ((await db.prepare('SELECT id FROM license_codes WHERE code = ?').get(code)) && attempts < 10);

  const result = await db.prepare(
    'INSERT INTO license_codes (code, user_id, duration_months) VALUES (?, ?, ?)'
  ).run(code, user_id || null, duration_months);

  res.status(201).json(await db.prepare('SELECT * FROM license_codes WHERE id = ?').get(result.lastInsertRowid));
});

// Alle Lizenzcodes auflisten
router.get('/license-codes', requireAuth, requireAdmin, async (req, res) => {
...
router.delete('/license-codes/:id', requireAuth, requireAdmin, async (req, res) => {
  await db.prepare('DELETE FROM license_codes WHERE id = ? AND used = 0').run(req.params.id);
  res.json({ success: true });
});
